	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				fixed4 normalC = 1;
				normalC.a *= tex2D(_MainTex, IN.texcoord).a;
				fixed4 rShifted = (tex2D(_MainTex, float2(IN.texcoord.x + rShiftRandom, IN.texcoord.y + rShiftRandom)).a > 0) ? 1 : 0;
				rShifted.a *= tex2D(_MainTex, IN.texcoord).a;
				fixed4 gShifted = (tex2D(_MainTex, float2(IN.texcoord.x + gShiftRandom, IN.texcoord.y + gShiftRandom)).a > 0) ? 1 : 0;
				gShifted.a *= tex2D(_MainTex, IN.texcoord).a;
				fixed4 bShifted = (tex2D(_MainTex, float2(IN.texcoord.x + bShiftRandom, IN.texcoord.y + bShiftRandom)).a > 0) ? 1 : 0;
				bShifted.a *= tex2D(_MainTex, IN.texcoord).a;
